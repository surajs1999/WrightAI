  ...
  return (
    <div style={{ position: "relative", marginTop: 16 }}>
      ...
      <div style={{ background: "#07051a", border: "1px solid rgba(175,169,236,0.09)", borderRadius: 10, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "8px 12px", borderBottom: "1px solid rgba(175,169,236,0.06)", background: "rgba(255,255,255,0.02)" }}>
...
const CARDS = [
  {
    accent: "#007ACC", accentBg: "rgba(0,122,204,0.08)", accentBorder: "rgba(0,122,204,0.25)",
    icon: (
    ...
    label: "VS Code Extension", sublabel: "One-click install",
  ...
  },
  {
    ...
    compat: null, badge: null, cta: null,
    code: "wright init .\nwright generate src/\nwright coverage --report\nwright chat",
    prompt: "pip install wright",
  },
  {
    accent: "#EF9F27", accentBg: "rgba(239,159,39,0.07)", accentBorder: "rgba(239,159,39,0.25)",
    icon: (
    ...
    label: "GitHub Action", sublabel: "Enforce in CI",
  ...
  {
    accent: "#00D4FF", accentBg: "rgba(0,212,255,0.06)", accentBorder: "rgba(0,212,255,0.22)",
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#00D4FF" strokeWidth="1.8" strokeLinecap="round">
        <rect x="3" y="3" width="18" height="18" rx="3"/><path d="M9 9h6M9 12h6M9 15h4"/>
      </svg>
    ),
    label: "MCP Server", sublabel: "Live AI context",
...
export default function GetStarted() {
  return (
    <section id="install" className="v2-section" style={{ background: "var(--bg)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: -120, left: "50%", transform: "translateX(-50%)", width: 900, height: 360, background: "radial-gradient(ellipse at center top, rgba(29,158,117,0.06) 0%, transparent 70%)", pointerEvents: "none", zIndex: 0 }} />

      {/* Terminal scan lines */}
      <div style={{ position: "absolute", inset: 0, backgroundImage: "repeating-linear-gradient(0deg, rgba(29,158,117,0.035) 0px, rgba(29,158,117,0.035) 1px, transparent 1px, transparent 28px)", pointerEvents: "none", zIndex: 0 }} />

      <div style={{ maxWidth: 1600, margin: "0 auto", position: "relative", zIndex: 1 }}>

        {/* Header — new heading, same style as other v2 sections */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ marginBottom: 56 }}>
          ...
          <h2 style={{ fontFamily: "var(--font-heading)", fontWeight: 800, fontSize: "clamp(30px, 3.2vw, 52px)", color: "var(--text)", letterSpacing: "-0.03em", lineHeight: 1.08, marginBottom: 16 }}>
            Up and running{" "}
            <span style={{ color: "var(--cyan)" }}>in minutes.</span>
          </h2>
          <p style={{ fontFamily: "var(--font-body)", fontSize: 16, color: "var(--text-muted)", maxWidth: 460, lineHeight: 1.75 }}>
            Four integration points. Pick one or use them all.
          </p>
        </motion.div>

        {/* Cards — exact same grid as original InstallGrid */}
        <div className="install-cards">
          {CARDS.map((card, i) => (
            ...
            >
              {/* Accent top bar */}
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg, transparent 0%, ${card.accent} 40%, ${card.accent} 60%, transparent 100%)`, opacity: 0.7 }} />

              {/* Icon + label */}
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: card.accentBg, border: `1px solid ${card.accentBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  {card.icon}
                </div>
                <div>
                  <div style={{ fontFamily: "var(--font-body)", fontWeight: 700, fontSize: 15.5, color: "var(--text)", lineHeight: 1.2 }}>{card.label}</div>
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: card.accent, letterSpacing: "0.04em", marginTop: 2 }}>{card.sublabel}</div>
                </div>
              </div>

              <div style={{ height: 1, background: "rgba(175,169,236,0.07)", margin: "16px 0" }} />

              <p style={{ fontFamily: "var(--font-body)", fontSize: 14, color: "var(--text-muted)", lineHeight: 1.7, marginBottom: 16 }}>{card.desc}</p>

              {card.points && (
                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
                  {card.points.map((pt, j) => (
                    <div key={j} style={{ display: "flex", alignItems: "flex-start", gap: 9 }}>
                      <div style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, marginTop: 1, background: card.accentBg, border: `1px solid ${card.accentBorder}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <svg width="8" height="8" viewBox="0 0 10 10" fill="none" stroke={card.accent} strokeWidth="2.2" strokeLinecap="round"><path d="M1.5 5l2.5 2.5 4.5-4.5"/></svg>
                      </div>
              ...
              {card.compat && (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
                  {card.compat.map((c: string) => (
                    <span key={c} style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: card.accent, padding: "4px 10px", background: card.accentBg, border: `1px solid ${card.accentBorder}`, borderRadius: 999 }}>{c}</span>
                  ))}
                </div>
              )}
              ...
              {card.badge && (
                <div style={{ marginTop: 16 }}>
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: card.accent, padding: "4px 12px", background: card.accentBg, border: `1px solid ${card.accentBorder}`, borderRadius: 999 }}>{card.badge}</span>
                </div>
              )}
              ...
              {card.cta && (
                ...
                >
                  <svg width="16" height="16" viewBox="0 0 100 100" fill="none">
                    <defs><linearGradient id="vsc-cta2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2AABEE"/><stop offset="100%" stopColor="#0070C5"/></linearGradient></defs>
                    <path fill="url(#vsc-cta2)" d="M73 4L38 30L16 17L5 23L5 77L16 83L38 70L73 96L95 85L95 15Z M73 73L38 50L73 27Z"/>
                  </svg>
                  {card.cta.text}
